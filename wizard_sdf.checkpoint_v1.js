...
(() => {
  ...
  function renderWizard(pose) {
    ...
    for (let y = 0; y < WIZ_H; y++) {
      for (let x = 0; x < WIZ_W; x++) {
        ...
        // CAPE REFINED (Multi-segment)
        // Transform World Nodes to Local Render Space
        if (G.player && G.wizard.cape.nodes.length > 0) {
          const p = G.player;
          const screenNodes = G.wizard.cape.nodes.map(n => ({
            x: (n.x - p.x) * wizard.facing + rx, // Map to render space relative to root X
            y: (n.y - p.y) + ry // Map to render space relative to root Y
          }));

          // Draw segments
          let dCape = 100;
          // Anchor at chest
          let prev = { x: capeAnchor.x, y: capeAnchor.y };

          for (let i = 0; i < screenNodes.length; i++) {
            const curr = screenNodes[i];
            // Tapering radius: 4.5 -> 2.5
            const t = i / (screenNodes.length - 1);
            const rad = lerp(4.5, 2.5, t);

            const dSeg = sdSegment(lx, ly, prev.x, prev.y, curr.x, curr.y) - rad;
            dCape = smin(dCape, dSeg, 1.0); // Smooth blend
          ...
          if (dCape < 0) {
            col = COL.robeDark;
            if (dCape < -1.5) col = COL.robeMid; // Inner fold
          }
        }
  ...
  function updatePhysics(dt, p) {
    ...
    // 5-Node Chain in World Space
    // Initialize if needed
    if (!wizard.cape.initialized) {
      wizard.cape.nodes = [];
      for (let i = 0; i < 5; i++) {
        wizard.cape.nodes.push({ x: p.x - f * 4, y: p.y + i * 5, oldX: p.x - f * 4, oldY: p.y + i * 5 });
      }
      wizard.cape.initialized = true;
    }

    const t = Date.now() * 0.003;
    const flutterX = Math.sin(t * 3.5) * 2.0;

    // Anchor Point (World Space) - Behind the neck
    const anchor = { x: p.x - f * 6, y: p.y - 20 }; // Adjusted to match chest height roughly

    // Forces
    const grav = 900 * dt * dt;
    const windX = Math.sin(t * 2) * 10 * dt * dt; // Ambient wind

    const nodes = wizard.cape.nodes;
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      // Fluid Drag
      // If player moves Right, nodes stay put (drag Left relative to player).
      // Since we are in World Space, we don't ADD player drag. World space nodes stay still if not pulled.
      // We only add AIR RESISTANCE.
      // Air resistance depends on velocity.

      let vx = (n.x - n.oldX) * 0.98; // Very Low damping for fluidity
      let vy = (n.y - n.oldY) * 0.98;

      n.oldX = n.x;
      n.oldY = n.y;

      n.x += vx + windX + flutterX * dt * dt * (i + 1); // Flutter increases at tip
      n.y += vy + grav;
    }

    // Constraints (Iterate multiple times for stiffness)
    const segLen = 7;
    for (let iter = 0; iter < 3; iter++) {
      ...
      if (dist > segLen) {
        let scale = segLen / dist;
        n.x = anchor.x + dx * scale;
        n.y = anchor.y + dy * scale;
      }
      ...
      for (let i = 0; i < nodes.length - 1; i++) {
        ...
        if (dist > segLen) {
          let scale = segLen / dist;
          // Move both towards center? Or pull child?
          // Pull child mostly for hanging cloth?
          // Let's relax both slightly
          let diff = dist - segLen;
  ...
  G.updateWizardAnim = function (dt) {
    ...
    // Turn detection: No hacks needed for cape anymore!
    if (prevFacing !== wizard.facing && p.onGround) {
      spawnDust(0, 4, COL.dust);
    }
