...
(() => {
  ...
  const wizard = {
    ...
    cape: {
      mid: { x: 0, y: 0, oldX: 0, oldY: 0 },
      tip: { x: 0, y: 0, oldX: 0, oldY: 0 }
    },
    beard: { ang: 0, vel: 0 },
  ...
  function renderWizard(pose) {
    ...
    for (let y = 0; y < WIZ_H; y++) {
      for (let x = 0; x < WIZ_W; x++) {
        ...
        // CAPE REFINED
        // We use the curve distance but modulate thickness
        // To estimate 't' for tapering is hard with pure SDF, so we just use thinner base + local shading
        const dCapeBase = sdCurve3(lx, ly, capeAnchor, cp2, cp3);
        const dCape = dCapeBase - 4.0; // Thinner (was 6.0)

        if (dCape < 0) {
          col = COL.robeDark;
          // Interior definition (fold)
          const dFold = dCapeBase - 1.5;
          // A lighter strip in the middle? or darker edge?
          // Let's make the center slightly lighter
          if (dFold < 0) col = COL.robeMid;
        }
  ...
  function updatePhysics(dt, p) {
    // CAPE - VERLET INTEGRATION (Fluid Cloth)
    const f = wizard.facing;
    const t = Date.now() * 0.003;
    const flutterX = Math.sin(t * 3.5) * 2.0;

    // Forces (Wind, Gravity, Inertia via player accel)
    // Note: In Verlet, inertia comes for free if we move the anchor!
    // But since we simulate in "Local Relative Space", we must add "Inertial Forces" manually
    // OR we transform to World Space? No, local offset is easier.

    // Inertia: if player accelerates Right, cape feels force Left.
    // p.vx is speed. Change in speed is accel. 
    // Simplified: Just add Drag based on velocity.

    // Gravity (Down)
    const grav = 900 * dt * dt;
    // Wind/Drag (Opposite to velocity)
    const dragX = -p.vx * 0.08 * dt * dt + flutterX * dt * dt * 20;
    const dragY = -p.vy * 0.08 * dt * dt;

    // Verlet Step
    const nodes = [wizard.cape.mid, wizard.cape.tip];
    for (let n of nodes) {
      const vx = (n.x - n.oldX) * 0.92; // Damping (0.92 = silky air resistance)
    ...
    // CONSTRAINTS
    // Anchor (0,0) -> Mid -> Tip
    const anchor = { x: -f * 4, y: 0 }; // slight offset behind neck
    const len1 = 14;
    const len2 = 14;

    // Constraint 1: Anchor -> Mid
    let dx = wizard.cape.mid.x - anchor.x;
    let dy = wizard.cape.mid.y - anchor.y;
    let dist = Math.sqrt(dx * dx + dy * dy);
    if (dist > len1) {
      const scale = len1 / dist;
      wizard.cape.mid.x = anchor.x + dx * scale;
      wizard.cape.mid.y = anchor.y + dy * scale;
    }

    // Constraint 2: Mid -> Tip
    dx = wizard.cape.tip.x - wizard.cape.mid.x;
    dy = wizard.cape.tip.y - wizard.cape.mid.y;
    dist = Math.sqrt(dx * dx + dy * dy);
    if (dist > len2) {
      const scale = len2 / dist;
      wizard.cape.tip.x = wizard.cape.mid.x + dx * scale;
      wizard.cape.tip.y = wizard.cape.mid.y + dy * scale;
    }

    // Beard Physics (Keep simple spring for beard, it's stiff hair)
  ...
  G.updateWizardAnim = function (dt) {
    ...
    if (prevFacing !== wizard.facing && p.onGround) {
      ...
      // INVERT positions to local space of new facing
      // Since it's Verlet, we just move X and oldX.
      wizard.cape.mid.x *= -0.9;
      wizard.cape.mid.oldX *= -0.9;
      wizard.cape.tip.x *= -0.9;
      wizard.cape.tip.oldX *= -0.9;

      // ADD SWING IMPULSE (modify oldPos to create velocity)
      // v = x - oldX  =>  newV = v + impulse
      // x - oldX_new = x - oldX + impulse
      // oldX_new = oldX - impulse
      const impulse = wizard.facing * 15; // Kick it back against turn
      wizard.cape.mid.oldX += impulse * 0.5;
      wizard.cape.tip.oldX += impulse;
    }

    const onGround = p.onGround;
    const speed = Math.abs(p.vx);

    // State Logic with Hysteresis for Ground
    // Prevent flickering between idle/fall if physics jitters on slope
    let state = 'idle';
    if (!onGround) {
      // Only switch to jump/fall if we've been airborne for a tiny bit or moving fast vertical
      // This prevents 1-frame "fall" states when walking down a slope pixel
      if (p.airTime > 0.05 || p.vy < -100) state = 'jump';
      else state = wizard.lastGroundedState || 'idle'; // Stick to last state? Or just walk?
      // Actually simpler: Just delay the 'jump' state animation slightly?
      // Let's use existing logic but checking airTime if available, or just vy.
      if (Math.abs(p.vy) > 20) state = 'jump';
      else state = (Math.abs(p.vx) > 5) ? 'walk' : 'idle'; // Treating micro-fall as walk/idle
    }
  ...
  G.drawWizard = function (screenX, screenY) {
    ...
    // ROUND coordinates to integers to prevent sub-pixel trembling
    const dx = Math.floor(screenX);
